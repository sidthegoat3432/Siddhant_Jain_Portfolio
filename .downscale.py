...
def save_png(path, w, h, ch, rows):
    ...
    def chunk(typ, payload):
        ...
        return c
    ihdr = struct.pack(">IIBBBBB", w, h, 8, {1: 0, 3: 2, 2: 4, 4: 6}[ch], 0, 0, 0)
    out = b"\x89PNG\r\n\x1a\n" + chunk(b"IHDR", ihdr) + chunk(b"IDAT", zlib.compress(bytes(raw), 9)) + chunk(b"IEND", b"")
    with open(path, "wb") as f:
        f.write(out)

w, h, ch, rows = load_png(sys.argv[1])
print("loaded", w, "x", h, "channels", ch)
# 2x box-average downscale
nw, nh = w // 2, h // 2
out_rows = []
for y in range(nh):
    row = bytearray(nw * ch)
    for x in range(nw):
        for c in range(ch):
            v = (rows[2 * y][(2 * x) * ch + c] + rows[2 * y][(2 * x + 1) * ch + c] +
                 rows[2 * y + 1][(2 * x) * ch + c] + rows[2 * y + 1][(2 * x + 1) * ch + c]) // 4
            row[x * ch + c] = v
    out_rows.append(bytes(row))
save_png(sys.argv[2], nw, nh, ch, out_rows)
print("saved", nw, "x", nh)
